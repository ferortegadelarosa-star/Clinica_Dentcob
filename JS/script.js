...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cookieAcceptBtn = document.getElementById('cookie-accept-all');
    const cookieRejectBtn = document.getElementById('cookie-reject');

    if (cookieOverlay) {
        // Check if user already made a choice
        const cookieConsent = localStorage.getItem('dentcob_cookie_consent');

        if (cookieConsent) {
            // User already consented — hide banner immediately
            cookieOverlay.style.display = 'none';

            // If they accepted analytics, load Google Analytics
            if (cookieConsent === 'all') {
                loadGoogleAnalytics();
            }
        } else {
            // Show the banner (block scrolling)
        ...
        if (cookieAcceptBtn) {
            cookieAcceptBtn.addEventListener('click', () => {
                localStorage.setItem('dentcob_cookie_consent', 'all');
                closeCookieBanner();
                loadGoogleAnalytics();
            });
        }
        ...
        if (cookieRejectBtn) {
            cookieRejectBtn.addEventListener('click', () => {
                localStorage.setItem('dentcob_cookie_consent', 'essential');
                closeCookieBanner();
            });
        }
    }
    ...
    function loadGoogleAnalytics() {
        // ──────────────────────────────────────────
        // INSTRUCCIONES: Reemplaza 'G-XXXXXXXXXX' con tu ID real de Google Analytics 4
        // Puedes obtenerlo en: https://analytics.google.com → Admin → Data Streams
        // ──────────────────────────────────────────
        const GA_ID = 'G-XXXXXXXXXX'; // ← Sustituir por tu ID real
